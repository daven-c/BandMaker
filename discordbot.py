...
async def help(interaction: discord.Interaction):
    ...
    embed.add_field(name='General', inline=False, value='**/online**: check if client is online')
    commands_str = '\n'.join([f'**/{cmd.name}**: {cmd.description}' for cmd in tree.get_commands() if cmd.name not in PRIV_METHODS])
...
async def show_list(interaction: discord.Interaction):
    embed = discord.Embed(title=f'{interaction.user.name}', colour=discord.Colour.red())
    
    stockList = []
    for row in cur.execute("SELECT stockID FROM stocks"):
        stockList.append(row[0])
    embed.add_field(name='Watch List', inline=False, value='\n'.join(stockList))
