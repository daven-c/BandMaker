...
def simulate_trading(pattern_matchers: List[PatternMatcher], data: pd.DataFrame, starting_cash: int = 1000, buy_ratio: float = 1.0, sell_ratio: float = 1.0, print_trades: bool = False):
    ...
    if print_trades:
        print(f"{'Date':<15} {'Pattern':<15} {'Buy/Sell':<10} {'Shares':<10} {'Price':<10} {'Value':<10} ${'Cash':<10}")
    ...
    for day_idx in range(len(data) - 1):
        for pattern in pattern_matchers:
            ...
            result = pattern.process(data=data.iloc[day_idx - pattern.CANDLES_REQUIRED + 1:day_idx + 1])  # Check if pattern can be detected from past day
            if result != []:  # pattern found
                ...
                if result[0][1] == 1:  # Bullish pattern found
                    # Buy the stock
                    shares_bought = int(current_value * buy_ratio / next_day_price)  # Allows change in purchasing strategy
                    cost = round(shares_bought * next_day_price, 2)
                    shares += shares_bought
                    current_value = round(current_value - cost, 2)
                    trades_made += 1
                    if print_trades:
                        print(f"\033[92m{next_day_date:<15} {pattern.type:<15} {'Buy':<10} {shares_bought:<10} ${next_day_price:<10} ${cost:<10} ${current_value:<10}\033[0m")
                elif result[0][1] == -1:
                    # Sell the stock
                    shares_sold = int(shares * sell_ratio)  # Allows change in purchasing strategy
                    profit = round(shares_sold * next_day_price, 2)
                    shares -= shares_sold
                    current_value = round(current_value + profit, 2)
                    trades_made += 1
                    if print_trades:
                        print(f"\033[91m{next_day_date:<15} {pattern.type:<15} {'Sell':<10} {shares_sold:<10} ${next_day_price:<10} ${profit:<10} ${current_value:<10}\033[0m")
    ...
    trades_made += 1
    if print_trades:
        print(f"\033[91m{formatted_date:<15} {'Final':<15} {'Sell':<10} {shares_sold:<10} ${current_day_price:<10} ${profit:<10} ${current_value:<10}\033[0m")
...
if __name__ == '__main__':
    ...
    result = simulate_trading(pattern_matchers, data, starting_cash=1000, buy_ratio=.2, sell_ratio=.2, print_trades=print_trades)
